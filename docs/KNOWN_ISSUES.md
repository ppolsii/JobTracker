...
**Reviewed (Version 2, Phase 22) - no code change required.** `ApplicationRepository.list` selects `companies!inner(name), cv_versions!inner(name)` to display names and to support server-side sorting by Company (`.order("name", { referencedTable: "companies" })`). `applications.company_id`/`cv_version_id` are constrained via **composite** foreign keys (`(company_id, user_id) REFERENCES companies (id, user_id)`), not plain single-column FKs.

Re-examined against PostgREST's documented composite-FK embedding behavior: `applications` has exactly one FK constraint targeting `companies` (`applications_company_owner_fk`, spanning `company_id, user_id`) and exactly one targeting `cv_versions` (`applications_cv_version_owner_fk`) - no other FK path connects either table pair, so there is no relationship ambiguity for PostgREST to resolve, even though `user_id` also participates in a separate, single-column FK to `users`. PostgREST embeds a composite FK by joining on every column of the constraint (`company_id = companies.id AND user_id = companies.user_id`), which is exactly the ownership match this schema already relies on elsewhere; `unique (id, user_id)` on both `companies` and `cv_versions` guarantees at most one matching row, consistent with `!inner`'s single-row expectation. On this reasoning, the query is expected to behave correctly as written, so per `IMPLEMENTATION_ORDER_V2.md` Phase 22 ("if an embedded query already behaves correctly, leave it unchanged"), `ApplicationRepository.list` was left unmodified.

This is a reasoned conclusion, not a live-executed one - no confirmed, reachable Supabase test account has existed in this environment at any point in this project (same root cause as every other "not verified against live data" entry in this file). Should still be the first thing manually confirmed once a real account is available. The documented fallback remains available if that confirmation ever contradicts this conclusion: replace the embedded select with two extra `.in("id", [...])` queries (fetch the page of applications first, collect distinct `company_id`/`cv_version_id`, then fetch names separately and merge in `ApplicationService.list`) - a pattern already proven safe elsewhere in this codebase (`CompanyRepository`/`CVVersionRepository`'s existing `.in()`-style lookups). Sorting by Company/CV Version name would then need to fall back to a client-side sort of the current page only (with a clear UI caveat), until either this is fixed or a later phase introduces a dedicated read view.
